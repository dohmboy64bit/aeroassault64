...
- **Repo-root CMake (VS / presets):** **`CMakeLists.txt`** + **`CMakePresets.json`** — presets **`engine-superbuild-ninja-release`**, **`engine-superbuild-vs2022-release`**, **`engine-superbuild-ninja-release-no-mm`**, **`engine-superbuild-ninja-release-afa-product`**, **`engine-superbuild-vs2022-release-afa-product`** configure **`build-root/`**, **`build-root-vs2022/`**, **`build-root-no-mm/`**, **`build-root-afa-product/`**, or **`build-root-vs2022-afa-product/`** and drive **`ExternalProject_Add(zelda64recomp_engine)`** into **`build-engine/`** (same inner **`CMAKE_SOURCE_DIR`** as direct **`-S lib/Zelda64Recomp`**).
...
- **Manual (same layout):**  
  ...
  then **`cmake --build build-engine`** (after satisfying **BUILDING.md** prerequisites). For **MM CPU** output **`RecompiledFuncs/*.c`** from **`us.rev1.toml`**, run **`tools/phase6_n64recomp_mm.ps1`**; for **RSP** **`rsp/*.cpp`**, run **`tools/phase6_rsprecomp_engine.ps1`** (**`tools/README.txt`**).
...
Repo-root: **`cmake --preset engine-superbuild-ninja-release-afa-product`** or **`engine-superbuild-vs2022-release-afa-product`** (root **`CMakeLists.txt`** forwards **`-DAEROASSAULT64_AFA_PRODUCT=ON`**). See **`config/afa_rsp/README.txt`**.

**AFA product + real `PatchesLib`:** when **`patches.elf`**, **`patches.toml`**, and **`RecompiledPatches/`** from **`N64Recomp`** are present for AFA, add **`-DAEROASSAULT64_AFA_RETAIL_PIPELINES=ON`** (with **`AFA_PRODUCT`**) — **`.\tools\phase6_engine_cmake.ps1 -Mode Configure -AfaProduct -AfaRetailPipelines`** or presets **`engine-superbuild-ninja-release-afa-product-retail`**, **`engine-superbuild-vs2022-release-afa-product-retail`**. See **`lib/Zelda64Recomp/AFA_PORT.md`** §2.

**Windows full link note:** upstream **`lib/Zelda64Recomp/CMakeLists.txt`** fetches the **Visual C++** SDL2 development ZIP and links **MSVC-built** **`freetype.lib`** and DXC/D3D12 paths. A **Ninja + MinGW** configure (e.g. WinLibs GCC) can compile most of the tree but the final **`Zelda64Recompiled.exe`** link often fails on those MSVC artifacts. For a full PE link on Windows, prefer **Visual Studio 2022**: repo script **`tools/phase6_engine_cmake_vs2022.ps1`** (**`build-engine-vs2022/`**), or **`cmake -G "Visual Studio 17 2022" -A x64 …`**, or preset **`engine-superbuild-vs2022-release`**, per **`lib/Zelda64Recomp/BUILDING.md`**.
