...
- **Repo-root CMake (VS / presets):** **`CMakeLists.txt`** + **`CMakePresets.json`** — presets **`engine-superbuild-ninja-release`**, **`engine-superbuild-vs2022-release`**, **`engine-superbuild-ninja-release-no-mm`**, and **`engine-superbuild-ninja-release-afa-product`** configure **`build-root/`**, **`build-root-vs2022/`**, **`build-root-no-mm/`**, or **`build-root-afa-product/`** and drive **`ExternalProject_Add(zelda64recomp_engine)`** into **`build-engine/`** (same inner **`CMAKE_SOURCE_DIR`** as direct **`-S lib/Zelda64Recomp`**).
