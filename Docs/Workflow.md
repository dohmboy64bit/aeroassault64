...
| Disagreements vs current `config/splat.yaml` | **IPL3:** none (see prior row). **Entry / main / BSS:** Ghidra matches current yaml + symbols — **no change required** from this snippet alone. **ROM header:** **`Load_Address` = `0x80200050`** matches **`entry`**. **`Release_Offset` `0x1447`:** still to reconcile with public ROM field definitions vs splat **`entry` @ ROM `0x1000`** (not asserted equal here). |
