...
| 6 | Engine + patches | **In progress** — **Scaffolding complete:** Aero fork CMake (**`AEROASSAULT64_*`**, **`AFA_PRODUCT`**, presets, CI **`engine-windows`** with **`-AfaProduct`**), stub Patches/RSP (**`tools/phase6_materialize_no_mm_engine_files.py`** / **`.ps1`**), Phase 5 **`RecompiledFuncs`** path (**`config/aerofighters_assault.n64recomp.toml`**), smoke docs (**`Docs/Debugging.md`**). **AFA static recomp bundle:** **`tools/phase6_full_recomp_afa.ps1`** / **`make phase6-full-recomp-afa-win`** chains **CPU N64Recomp** + **RSP RSPRecomp** after **`make strict-verify`**. **AFA RSP C++ from retail ROM:** committed **`config/afa_rsp/*.template.toml`** + engine TOMLs + **`make phase6-rsprecomp-afa`** / **`tools/phase6_rsprecomp_afa.ps1`** (see **`lib/Zelda64Recomp/AFA_PORT.md`** §1); Ghidra **`RSPRecomp_Confirm_Findings.py`** / **`RSPRecomp_AFA_AllInOne.py`** for prefix/xref hints — **Listing / DMA / `OSTask`** proof remains manual where noted. **Full AFA retail fork still open** — **`patches.elf`** / **`patches.toml`** / **`AFA_RETAIL_PIPELINES`**, replace MM **`src/game/`**, engine **`patches/`** / real **`RecompiledPatches/`**, **`assets/`** (Phase 6 checklist **full retail** row and fork touchpoints table). |
...
## Phase audit (automation vs. manual)

Use this table to see what **CI / `make check`** already proves vs. what stays **owner + Ghidra + game QA**.

| Phase | Automated (local or GitHub Actions) | Manual / product (cannot be closed by script alone) |
|-------|-------------------------------------|--------------------------------------------------------|
| **1** | — (checklist + host log in this file) | MIPS GCC optional row; legal ROM acquisition |
| **2** | — | Ghidra import + findings log; **`tools/ghidra/Phase2_Closeout_Report.py`** |
| **3** | **`make verify-rodata-sync`** / **`python3 tools/verify_rodata_splits_sync.py`** — **`tools/ghidra/Phase3_Closeout_Report.py`** **`RODATA_ROM_SPLITS`** ↔ **`config/splat.yaml`** (part of **`make check`** / **`repo-check`**) | Optional Ghidra xref review of **`rodata`** boundaries (see Phase 3 closeout **Still to do**) |
| **4** | **`make strict-verify`** after each **`splat split`** (WSL + **`asm/`** + MIPS **`ld`**) | — |
| **5** | **`config/aerofighters_assault.n64recomp.toml`** validated by **`verify_n64recomp_toml.py`** (in **`make check`**) | **`tools/N64Recomp.exe`** exit **0** on your ELF; extend **`stubs`** / patches after large **`asm/`** churn |
| **6** | **`make check`**, **`verify-phase6-layout`**, **`engine-windows`** (MSVC **`-AfaProduct`** → **RecompiledFuncs** + **PatchesLib**); **`make phase6-full-recomp-afa-win`** when ELF + ROM + RSP TOMLs present (CPU + RSP); **`make phase6-rsprecomp-afa`** for RSP only | **Full AFA retail** row (game/UI, **`patches.elf`** + **`patches.toml`** + **`AFA_RETAIL_PIPELINES`**, assets); **Listing / DMA / `OSTask`** for RSP **`text_offset`** beyond template + scans (**`AFA_PORT.md`** §1) |
| **7** | Same **`engine-windows`** MSVC configure/build slice | Full **`Zelda64Recompiled.exe`** link on a dev machine (**`tools/phase6_engine_cmake_vs2022.ps1`**) |
| **8** | **`repo-check`** + **`engine-windows`** on **`master`** / **`main`** / PRs | **Product QA** row (gameplay, ROM-load matrix) |

**Last audited on this tree:** **`make check`** exit **0** (WSL) **2026-05-13** — splat/Makefile sync, rodata tuple (30 offsets), entrypoint triple, N64Recomp TOML, phase6 layout, **`py_compile`** on all repo-root **`tools/*.py`**.
...
- **ROM-free validation:** **`make check`** — runs **`verify-splat-makefile-sync`**, **`verify-rodata-sync`**, **`verify-entrypoint-sync`** (splat **`entry`** **`vram`** / **`symbol_addrs.txt`** / N64Recomp **`[input].entrypoint`**), **`python3 tools/verify_n64recomp_toml.py`**, **`python3 tools/verify_phase6_layout.py`** (when **`lib/Zelda64Recomp`** exists, **`RecompiledFuncs/`** bridge must not split; see **`lib/README.txt`**), and **`python3 -m py_compile`** on every repo-root **`tools/*.py`** (see **`Makefile`** **`TOOLS_ROOT_PY`**; **`tools/ghidra/*.py`** are PyGhidra-only and not in that glob). Same Python checks run in **GitHub Actions** (`.github/workflows/repo-check.yml` on **`master`** / **`main`**). MSVC configure + **`RecompiledFuncs`** / **`PatchesLib`** compile (stub CPU output when needed) run in **`.github/workflows/engine-windows.yml`**. Requires **Python 3.11+** on the runner / host (**`tomllib`**). **`make help`** lists common targets.
...
9. **RSPRecomp `text_offset` hints (Phase 6 engine):** If memory blocks match Phase 2 (**`.rom`**, **`.ram`**), run **`tools/ghidra/RSPRecomp_AFA_AllInOne.py`** once in Ghidra for the full scripted bundle (same helpers as below; tunables remain in each **`tools/ghidra/RSP_*.py`** / **`Find_RSP_*.py`** — see **`tools/ghidra/README.txt`**), or run steps individually. Run **`tools/ghidra/RSPRecomp_Confirm_Findings.py`** with tunables from your emulator snapshot to **xref** key VRAMs and **search `.rom`** for an IMEM bootstrap prefix (**`EXPECTED_TEXT_*`** default to **aspMain** template; optional **`IMEM_BOOTSTRAP_NJPG_BE_U32`** + **`EXPECTED_NJPG_*`** for **njpgdsp** per **`config/afa_rsp/njpgdspMain.afa.us.template.toml`**). Run **`tools/ghidra/Find_RSP_Microcode_ROM_Hints.py`** in Ghidra (PyGhidra — same **`support/pyghidraRun.bat`** as **`Phase2_Closeout_Report.py`**). It prints a **histogram of ROM file offsets** referenced from RAM (operand xrefs + **`lui`/`addiu`** ROM pointers). Also run **`tools/ghidra/RSP_LibUltra_And_IMEM_Scan.py`** for symbol-name matches, **`0x04001000`**-style immediates, ASCII **`osSpTask`** / **`OSTask`**, and Paradigm **`uv*`** rodata hints when SDK strings are stripped. Then run **`tools/ghidra/RSP_Scheduler_String_Xref_Trace.py`** to **follow incoming xrefs** from those rodata seeds into **callers** and list **`.rom`** references from those functions (plus one **`jal`** callee hop). For **`0x04001000` / `0x04000FC0`**-style setup and **jal** windows around a DMA helper (e.g. **`FUN_80246fd0`**), run **`tools/ghidra/RSP_IMEM_Load_And_Helper_Call_Trace.py`** (set **`HELPER_ENTRY_VRAM`** in the script to that function’s entry). To **list `jal` callee entry addresses** from a driver function (e.g. **`FUN_8023d92c`** @ **`0x8023D92C`**), run **`tools/ghidra/RSP_List_Jal_Callees_From_Function.py`** and copy the printed **callee entry** values into **`HELPER_ENTRY_VRAM`**, or run **`tools/ghidra/RSP_Jal_Call_Sites_Disasm_From_Caller.py`** once to print **disassembly before every `jal`** from that caller (arg setup for all helpers), or **`tools/ghidra/RSP_Jal_Arg_Register_Slice.py`** for a **heuristic last-def** of **`a0`–`a3`** (and **`lw`** bases) at each **`jal`** to a chosen callee (e.g. **`FUN_80246fd0`**). When the slice names a **`jal`** whose **`v0`** feeds a saved reg (e.g. **`FUN_8023d820`** from **`jal @ 0x8023d944`** on AFA USA), run **`tools/ghidra/RSP_Function_Return_Reg_Slice.py`** with **`TARGET_FUNCTION_ENTRY_VRAM`** set to that callee’s entry to print **`v0`/`v1`** last-def before each **`jr ra`**, then run **`tools/ghidra/RSP_RAM_Context_Field_Xrefs.py`** with **`BASE_VRAM`** set to the **`lui`/`addiu`** return constant (e.g. **0x802839B0**) and **`FIELD_OFFSETS`** **`(0x8, 0xC)`** to list **incoming xrefs** (**`insn=LOAD`/`STORE`** on ref-source instructions), **big-endian stored words** (and **`.rom`** file-offset hints when the word is a cart pointer). With default tunables it also runs **constant-base `lw`/`sw`** + **`sw` xref** blocks (**`RUN_CONSTANT_BASE_MEMOPS`**, **`RUN_STORE_XREF_SCAN`**) matching Memops. **`tools/ghidra/RSP_RAM_Constant_Base_Memops.py`** scans **`lw`/`sw`** for the same **`BASE_VRAM`+offsets** when the memory base is a tracked **`lui`/`addiu`/`ori`** constant in-function (**`JAL_KNOWN_V0_BY_CALLEE_ENTRY`** for **`jal`** returns); set **`MEMOP_RUN`** to **`\"stores\"`**, **`\"stores_xref\"`** (xref **`sw`** to each EA), **`\"loads\"`**, or **`\"all\"`**. Use these scripts to **short-list** microcode blob locations; **confirm** in the Listing (DMA / **`OSTask`** / graphics init) before copying values into **`config/afa_rsp/*.template.toml`** — see **`lib/Zelda64Recomp/AFA_PORT.md`** §1. Optional cross-title sanity check: import **Pilotwings 64 USA** the same way and re-run the same Ghidra scripts — links and a **non-AFA** RSPRecomp example TOML are in **`Docs/RepoInjests/Pilotwings/README.txt`**.
...
- [x] **RODATA split list drift (stdlib):** **`python3 tools/verify_rodata_splits_sync.py`** / **`make verify-rodata-sync`** — ensures **`tools/ghidra/Phase3_Closeout_Report.py`** **`RODATA_ROM_SPLITS`** matches **`config/splat.yaml`** **`main`** **`rodata`** ROM starts (**30** entries; part of **`make check`** / **`.github/workflows/repo-check.yml`**; verified **2026-05-13**).
- [ ] *(Optional — does not block calling Phase 3 complete.)* Validate **`rodata`** split boundaries in Ghidra (xrefs from **`.text`** into **`.rom`** **0x52B90–0x57D20**); adjust **`config/splat.yaml`** if a boundary cuts a table. The drift check above is **not** a substitute for visual xref review.
...
| **AFA full static recomp (CPU + RSP)** | **`pwsh tools/phase6_full_recomp_afa.ps1`** — junction **`RecompiledFuncs`**, **`phase5_run_aero_n64recomp.ps1`**, **`phase6_rsprecomp_afa.ps1`** (**`lib/Zelda64Recomp/AFA_PORT.md`** top section). **`-WhatIf`**, **`-SkipN64Recomp`**, **`-SkipRSP`**, **`-CopyTools`**, **`-RomPath`**. PatchesLib still manual (**`patches.elf`**, **`patches.toml`**, **`-AfaRetailPipelines`**). |
...
- [x] **AFA RSPRecomp TOMLs + tooling (scaffolding — not full game port)** — committed **`config/afa_rsp/*.template.toml`** + engine **`aspMain.afa.us.toml`** / **`njpgdspMain.afa.us.toml`**; derive/re-validate offsets with **`tools/rsprecomp_scan_offsets.py`**, **`tools/rsprecomp_left_edge.py`**, **`tools/rsprecomp_find_njpg2.py`**, **`tools/rsprecomp_fine_njpg.py`**; optional host **`tools/rsp_rom_capstone_mips.py`**; Ghidra **`tools/ghidra/RSPRecomp_Confirm_Findings.py`** / **`RSPRecomp_AFA_AllInOne.py`** (**`tools/ghidra/README.txt`**); **`tools/phase6_full_recomp_afa.ps1`** (**`make phase6-full-recomp-afa-win`**) chains **CPU N64Recomp** + **RSP RSPRecomp**. **Listing / DMA / `OSTask`** hard proof remains manual per **`lib/Zelda64Recomp/AFA_PORT.md`** §1.
...
**Policy:** Phases **1–5** and **7–8 automation baseline** are **closed at the repo/CI level** as documented above. Phase **6** stays **in progress** until the **full retail runtime** checklist row is satisfied; the **scaffolding** and **AFA RSPRecomp TOMLs + tooling** rows above are **closed in-repo** (CI + docs + templates), while **full retail** (game/UI/patches/assets) remains open. Optional Phase **1–3** items (MIPS GCC, Ghidra rodata xref spot-check) remain **optional** and do not block Phase **6** retail closure.
