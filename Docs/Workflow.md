...
| 6 | Engine + patches | **In progress** — **`lib/Zelda64Recomp`** submodule on branch **`dev`** with Aero fork CMake (`AEROASSAULT64_*`, optional AFA **`GameEntry`**, **`recomp::start(Configuration)`** per **`lib/Zelda64Recomp/lib/N64ModernRuntime/librecomp/include/librecomp/game.hpp`**); no-MM stubs via **`tools/phase6_materialize_no_mm_engine_files.ps1`** + **`-DAEROASSAULT64_NO_MM_ROM=ON`** (**`lib/Zelda64Recomp/CMakeLists.txt`**). Full **`Zelda64Recompiled.exe`** still needs local N64Recomp output under **`RecompiledFuncs/`** (gitignored) plus **`lib/Zelda64Recomp/BUILDING.md`** RSP / patches when not using stubs. |
...
| 8 | Test / stabilize | **Started** — ROM-free **`repo-check`** + Windows **`engine-windows`** in **`.github/workflows/`**; gameplay / full-fork tests still open. |
...
| **Windows prep (all of the above)** | **`tools/phase6_setup_windows.ps1`** — runs **`phase6_link_recompiledfuncs.ps1`**, **`phase6_copy_n64recomp_to_engine.ps1`**, then **`python`/`python3 tools/verify_phase6_layout.py`**. Optional **`-RspRecomp`**: if **`lib/Zelda64Recomp/mm.us.rev1.rom_uncompressed.z64`** exists, runs **`tools/phase6_rsprecomp_engine.ps1`** ( **`lib/Zelda64Recomp/BUILDING.md`** section 4). |
...
| **No MM ROM (stub patches + RSP)** | **`tools/phase6_materialize_no_mm_engine_files.ps1`** copies stub **`patches_bin.h`**, **`recomp_overlays.inl`**, **`funcs.h`** into **`lib/Zelda64Recomp/RecompiledPatches/`** (paths **`register_patches.cpp`** includes). Then configure the engine with **`-DAEROASSAULT64_NO_MM_ROM=ON`** — see **`lib/Zelda64Recomp/CMakeLists.txt`** **`option(AEROASSAULT64_NO_MM_ROM …)`** and **`tools/phase6_engine_cmake.ps1 -NoMmRom`**. Repo-root preset **`engine-superbuild-ninja-release-no-mm`** forwards the same flag via **`CMakeLists.txt`**. Stock **`src/game/`** is still MM-oriented; this only bypasses **`patches/`** / **`N64Recomp patches.toml`** and missing **`rsp/*.cpp`**. |
...
| **Repo-root CMake (alternative)** | **`CMakeLists.txt`** at repo root uses **`ExternalProject_Add`** (**`zelda64recomp_engine`**) so the **inner** configure uses **`SOURCE_DIR = lib/Zelda64Recomp`** (same **`CMAKE_SOURCE_DIR`** invariant as direct **`-S lib/Zelda64Recomp`**). Outer binary dirs: **`build-root/`** (Ninja) or **`build-root-vs2022/`** (VS). **`CMakePresets.json`**: **`engine-superbuild-ninja-release`**, **`engine-superbuild-vs2022-release`**, **`engine-superbuild-ninja-release-no-mm`**. Example: **`cmake --preset engine-superbuild-ninja-release`** then **`cmake --build --preset engine-superbuild-ninja-release`**. Default **`ALL`** also builds **`zelda64recomp_engine`** via **`aero_phase6_engine_default`**. |
...
- [x] **Windows one-shot prep** — **`tools/phase6_setup_windows.ps1`** chains junction + PE copy + **`verify_phase6_layout.py`**; optional **`-RspRecomp`** (documented in **`Docs/Workflow.md`** Phase 6 table).
...
- [ ] **AFA fork** — replace MM **`us.rev1.toml`**, **`src/game/`**, RSP **`rsp/*.cpp`**, engine **`patches/`** / **`RecompiledPatches/`**, assets, and related glue so the shipped binary is **Aero Fighters Assault**, not stock **`Zelda64Recompiled`** (**`Docs/Workflow.md`** fork touchpoints table). *(Large port — not done in a single scripted pass.)*
- [x] First **PowerShell** / **VS** run of the built engine binary with **AFA** **`RecompiledFuncs`** + **`-NoMmRom`** — smoke notes under **`Docs/Debugging.md`** § *Phase 6 — No-MM + AFA `RecompiledFuncs` (MSVC smoke)*. A true **AFA**-branded runtime still depends on the **AFA fork** row above.
...
| Patch ELF + **`RecompiledPatches/`** | Engine **`patches/`** **`Makefile`** → **`patches.elf`**; **`./N64Recomp patches.toml`** → **`RecompiledPatches/patches.c`** etc. (**`CMakeLists.txt`** custom commands) | Not the same as repo-root **`patches/`** — see **`patches/README.txt`**. Needs an AFA **`patches.toml`** + MIPS patch sources under the **engine** tree. Interim bring-up: **`AEROASSAULT64_NO_MM_ROM`** + **`tools/phase6_no_mm_engine/`** stubs (**`lib/Zelda64Recomp/CMakeLists.txt`**) skip that pipeline. |
