...
- **ROM-free validation:** **`make check`** — runs **`verify-rodata-sync`** (Ghidra **`RODATA_ROM_SPLITS`** vs **`config/splat.yaml`**) and **`py_compile`** on **`tools/*.py`**. Same steps run in **GitHub Actions** (`.github/workflows/repo-check.yml` on **`master`** / **`main`**).
