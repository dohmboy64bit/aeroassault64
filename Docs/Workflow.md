...
- [ ] In WSL (optional): **`gcc-mips-linux-gnu`** → **`mips-linux-gnu-gcc`** for compiling **C** to MIPS objects (separate from **`binutils-mips-linux-gnu`**). Install in an **interactive** WSL shell (sudo prompts cannot be satisfied from this repo’s automation): `sudo apt-get update && sudo apt-get install -y gcc-mips-linux-gnu`, then `mips-linux-gnu-gcc --version`. Check this box after install. **Do not put sudo passwords in `Workflow.md` or chat logs.**
