...
| 6 | Engine + patches | **In progress** — **`lib/Zelda64Recomp`** submodule on branch **`dev`** with Aero fork CMake (`AEROASSAULT64_*`, optional AFA **`GameEntry`**, **`recomp::start(Configuration)`** per **`lib/Zelda64Recomp/lib/N64ModernRuntime/librecomp/include/librecomp/game.hpp`**); stub Patches/RSP via **`tools/phase6_materialize_no_mm_engine_files.ps1`** + **`-DAEROASSAULT64_NO_MM_ROM=ON`** or **`-DAEROASSAULT64_AFA_PRODUCT=ON`** (**`lib/Zelda64Recomp/CMakeLists.txt`**). Full **`Zelda64Recompiled.exe`** still needs local N64Recomp output under **`RecompiledFuncs/`** (gitignored) plus **`lib/Zelda64Recomp/BUILDING.md`** RSP / patches when not using stubs. |
...
| **AFA product (stub patches + RSP)** | Same **`phase6_materialize_no_mm_engine_files.ps1`** as no-MM (stub **`RecompiledPatches/`** headers). Configure with **`-DAEROASSAULT64_AFA_PRODUCT=ON`** — **`tools/phase6_engine_cmake.ps1 -AfaProduct`**; repo-root preset **`engine-superbuild-ninja-release-afa-product`** forwards **`CMakeLists.txt`**. Skips MM **`patches.elf`** / **`patches.toml`** like **`NO_MM_ROM`** but leaves **`AEROASSAULT64_NO_MM_ROM` off** for AFA-only bring-up; see **`config/afa_rsp/README.txt`**. |
...
| **Repo-root CMake (alternative)** | **`CMakeLists.txt`** at repo root uses **`ExternalProject_Add`** (**`zelda64recomp_engine`**) so the **inner** configure uses **`SOURCE_DIR = lib/Zelda64Recomp`** (same **`CMAKE_SOURCE_DIR`** invariant as direct **`-S lib/Zelda64Recomp`**). Outer binary dirs: **`build-root/`** (Ninja) or **`build-root-vs2022/`** (VS). **`CMakePresets.json`**: **`engine-superbuild-ninja-release`**, **`engine-superbuild-vs2022-release`**, **`engine-superbuild-ninja-release-no-mm`**, **`engine-superbuild-ninja-release-afa-product`**. Example: **`cmake --preset engine-superbuild-ninja-release`** then **`cmake --build --preset engine-superbuild-ninja-release`**. Default **`ALL`** also builds **`zelda64recomp_engine`** via **`aero_phase6_engine_default`**. |
