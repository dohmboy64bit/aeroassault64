...
| ROM header / `.rom` cart image | **Ghidra `.rom`:** **`ram:b0000000`–`ram:b07fffff`** — linear **cart ROM** view (`b0000000` + **file offset**). Standard **64-byte header**: **`Magic`** `80371240`, **`Load_Address`** @ ROM **`0x08`** = **`0x80200050`** (matches **`entrypoint` / `ramMain`** and splat **`entry` vram**). **`Game_Title`** @ **`0x20`**: `AERO FIGHTERS ASSAUL…`. **CRC1/CRC2** `1B598BF1` / `ECA29B45` per listing. **`Release_Offset`** shown as **`0x1447`** — compare to the ROM layout doc you trust (e.g. [N64brew — ROM](https://n64brew.dev/wiki/ROM)); **do not assume** it equals splat’s **`entry` ROM `0x1000`** without that check. Raw bytes from **`b0000040`** onward = cart **`0x40`** (**IPL3**), same bytes splat keeps as **`ipl3` `bin`**. |
...
| Disagreements vs current `config/splat.yaml` | **IPL3:** none (see prior row). **Entry / main / BSS:** Ghidra matches current yaml + symbols — **no change required** from this snippet alone. **ROM header:** **`Load_Address` = `0x80200050`** matches **`entry`**. **`Release_Offset` `0x1447`:** still to reconcile with public ROM field definitions vs splat **`entry` @ ROM `0x1000`** (not asserted equal here). **Tail @ ROM `0x57D20`:** user bytes show **MIPS code** at **`0x57D20`** while yaml uses **`bin`** from **`0x57D20`** — **treat as a splat segment fix for Phase 3**, not Phase 2 truth conflict with Ghidra. |
