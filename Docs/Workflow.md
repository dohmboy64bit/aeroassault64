...
| **AFA product (stub patches + RSP)** | Same **`phase6_materialize_no_mm_engine_files.ps1`** as no-MM (stub **`RecompiledPatches/`** headers). Configure with **`-DAEROASSAULT64_AFA_PRODUCT=ON`** — **`tools/phase6_engine_cmake.ps1 -AfaProduct`** or **`tools/phase6_engine_cmake_vs2022.ps1 -Mode Configure -AfaProduct`**; repo-root presets **`engine-superbuild-ninja-release-afa-product`**, **`engine-superbuild-vs2022-release-afa-product`**. Skips MM **`patches.elf`** / **`patches.toml`** like **`NO_MM_ROM`** but leaves **`AEROASSAULT64_NO_MM_ROM` off** for AFA-only bring-up; see **`config/afa_rsp/README.txt`**. |
...
| **Repo-root CMake (alternative)** | **`CMakeLists.txt`** at repo root uses **`ExternalProject_Add`** (**`zelda64recomp_engine`**) so the **inner** configure uses **`SOURCE_DIR = lib/Zelda64Recomp`** (same **`CMAKE_SOURCE_DIR`** invariant as direct **`-S lib/Zelda64Recomp`**). Outer binary dirs: **`build-root/`** (Ninja), **`build-root-vs2022/`** (VS), **`build-root-afa-product/`**, **`build-root-vs2022-afa-product/`**. **`CMakePresets.json`**: **`engine-superbuild-ninja-release`**, **`engine-superbuild-vs2022-release`**, **`engine-superbuild-ninja-release-no-mm`**, **`engine-superbuild-ninja-release-afa-product`**, **`engine-superbuild-vs2022-release-afa-product`**. Example: **`cmake --preset engine-superbuild-ninja-release`** then **`cmake --build --preset engine-superbuild-ninja-release`**. Default **`ALL`** also builds **`zelda64recomp_engine`** via **`aero_phase6_engine_default`**. |
