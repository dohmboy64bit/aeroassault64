...
- **Build:** from repo root, **`tools/phase6_materialize_no_mm_engine_files.ps1`**, then **`tools/phase6_engine_cmake_vs2022.ps1 -Mode All -NoMmRom`** or **`-Mode All -AfaProduct`** → **`build-engine-vs2022/Release/Zelda64Recompiled.exe`** (inner CMake **`VS_DEBUGGER_WORKING_DIRECTORY`** = **`lib/Zelda64Recomp/`** in **`lib/Zelda64Recomp/CMakeLists.txt`** — use the same cwd so relative **`assets/`** matches **`lib/Zelda64Recomp/BUILDING.md`** § 6).
